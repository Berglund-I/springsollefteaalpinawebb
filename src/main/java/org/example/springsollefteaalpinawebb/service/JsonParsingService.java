...
public class JsonParsingService {
    ...
    public List<FacebookPicture> parseJsonStringToPictures(String jsonString) {
        // Parse the JSON string into a JSONObject
        JSONObject jsonObject = new JSONObject(jsonString);
        ...
        // Create a list to store the FacebookPicture objects
        List<FacebookPicture> facebookPictures = new ArrayList<>();
        ...
        for (int i = 0; i < dataArray.length(); i++) {
            ...
            // Check if the 'created_time' and 'id' fields exist in the item
            if (dataItem.has("created_time") && dataItem.has("id")) {
                // Access the 'created_time' and 'id' fields of the item
                String createdTime = dataItem.getString("created_time");
                String id = dataItem.getString("id");

                // Create a new FacebookPicture object
                FacebookPicture facebookPicture = new FacebookPicture(id, createdTime);

                // Add the FacebookPicture object to the list
                facebookPictures.add(facebookPicture);
            }
        }

        // Return the list of FacebookPicture objects
        return facebookPictures;
    }
}
